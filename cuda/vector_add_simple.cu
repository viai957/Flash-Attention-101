...
{
    int i = threadIdx.x;
    if (i < N)   // this is a simple kernel, it assumes that the number of threads is equal to the number of elements in the array 
    // if the number of threads is greater than the number of elements, the kernel will not work correctly
    {
        OUT[i] = A[i] + B[i];
    }
}
